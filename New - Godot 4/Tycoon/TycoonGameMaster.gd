class_name TycoonGameMaster
extends Node

const center_card_spacing := 16

var match_player_standings 	:= preload("res://Tycoon/Standings/PlayerStanding.tscn")
var game_player_standings 	:= preload("res://Tycoon/Standings/GameEndPlayer.tscn")
...
func trade(cards : Array[Card]) -> bool:
	...
	if requestee != 0 and cards.size() == cards_needed:
		...
		var requestee_player : TycoonPlayer = player_helper.get_player_from_owner_id(requestee)
		requestee_player.connect("trade_finished", Callable(self, "announce_finish_trade").bind(requestee_player))
...
func announce_finish_trade(requestee_player : TycoonPlayer):
		requestee_player.disconnect("trade_finished", Callable(self, "announce_finish_trade"))
...
func end_match(winner_array : Array) -> void:
	...
	for i in range(winner_array.size()):
		...
		# Give points and check wincon
		if !(winner_array[i] in points):
			points[winner_array[i]] = 0
		
		if winners.size() < 4:
			if i == 0:
				points[winner_array[i]] += 2
			elif i != winners.size() - 1:
				points[winner_array[i]] += 1
		
			if points[winner_array[i]] >= 5:
				end_game()
				return
		else:
			if i == 0:
				points[winner_array[i]] += 3
			if i == 1:
				points[winner_array[i]] += 2
			elif i != winners.size() - 1:
				points[winner_array[i]] += 1
			
			if points[winner_array[i]] >= 7:
				end_game()
				return
		
		var player_standings_panel = match_player_standings.instantiate()
		player_standings_panel.setup(winner_array.size(), i + 1, player_name, points[winner_array[i]])
		match_standings.get_node("Layout/PlayerStandings").add_child(player_standings_panel)
	
	match_standings.get_node("Layout/Label").text = "Match %d Standings:" % match_num
...
func end_game():
	%WinnerDisplay/Game.show()
	
	var sorted_points := []
	for player in points:
		sorted_points.append([player, points[player]])
	sorted_points.sort_custom(Callable(self, "sort_score"))
	
	var game_standings : Array[int] = []
	for player in sorted_points:
		game_standings.append(player[0]) 
	
	var winner = %WinnerDisplay/Game/PanelContainer/Layout/GameEndWinner
	winner.get_node("PlayerName").text = NetworkSession.player_info[game_standings[0]]["name"]
	
	var loser = %WinnerDisplay/Game/PanelContainer/Layout/GameEndLoser
	loser.get_node("PlayerName").text = NetworkSession.player_info[game_standings[-1]]["name"]
	loser.get_node("PlayerStanding").text = "%d. " % winners.size()
	
	var other_players = %WinnerDisplay/Game/PanelContainer/Layout/OtherStandings
	for i in range(1, winners.size() - 1):
		var game_player = game_player_standings.instantiate()
		game_player.setup(i + 1, NetworkSession.player_info[game_standings[i]]["name"])
		other_players.add_child(game_player)
