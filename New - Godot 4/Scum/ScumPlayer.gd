class_name ScumPlayer
extends Node

var card_scn := preload("Card/Card.tscn")

@onready var game_master : ScumGameMaster = GameSync.get_node_in_current_scene("%GameMaster")
...
func play_turn(cards_to_play : Array[Card]) -> void:
	var turn_played_successfully : bool = await game_master.play_turn(cards_to_play)
